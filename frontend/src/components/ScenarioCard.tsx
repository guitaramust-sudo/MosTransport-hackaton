  ...
  return (
    <Pressable onPress={onPress} disabled={disabled} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
      <View style={styles.topRow}>
        <View style={[styles.icon, scenario.difficulty === 'РЎР»РѕР¶РЅС‹Р№' && styles.iconCritical]}>
          <Text style={styles.iconText}>{scenario.difficulty === 'РЎР»РѕР¶РЅС‹Р№' ? '!' : scenario.progress === 100 ? 'вњ“' : 'в†—'}</Text>
        </View>
        <View style={styles.meta}>
          <Text style={styles.tag}>{scenario.tag}</Text>
          <Text style={styles.duration}>{scenario.duration} В· {scenario.difficulty}</Text>
        </View>
        <Text style={styles.arrow}>вЂє</Text>
      </View>
