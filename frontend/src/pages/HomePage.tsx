  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.header}>
        <View>
          <Text style={styles.eyebrow}>Р”РћР‘Р РћР• РЈРўР Рћ</Text>
          <Text style={styles.name}>РђР»РµРєСЃРµР№, Рє СЃРјРµРЅРµ РіРѕС‚РѕРІ?</Text>
        </View>
        <Pressable style={styles.avatar} onPress={() => dispatch(navigate('profile'))}>
          <Text style={styles.avatarText}>РђРљ</Text>
      ...
      <View style={styles.hero}>
        ...
        <Text style={styles.heroLabel}>Р’РђРЁ РЈР РћР’Р•РќР¬</Text>
        <View style={styles.levelRow}>
          <Text style={styles.level}>7</Text>
          <View style={styles.levelCopy}>
            <Text style={styles.levelTitle}>РџСЂРѕРІРѕРґРЅРёРє-СЌРєСЃРїРµСЂС‚</Text>
            <Text style={styles.levelHint}>Р•С‰С‘ 260 XP РґРѕ РЅРѕРІРѕРіРѕ СѓСЂРѕРІРЅСЏ</Text>
          </View>
        </View>
        <View style={styles.xpTrack}><View style={styles.xpFill} /></View>
        <View style={styles.heroStats}>
          <Text style={styles.heroStat}>в—† {score} XP</Text>
          <Text style={styles.heroStat}>вљЎ 4 РґРЅСЏ РїРѕРґСЂСЏРґ</Text>
        </View>
      </View>

      <View style={styles.sectionHead}>
        <View>
          <Text style={styles.sectionTitle}>РџСЂРѕРґРѕР»Р¶РёС‚СЊ РѕР±СѓС‡РµРЅРёРµ</Text>
          <Text style={styles.sectionSubtitle}>РўСЂРµРЅРёСЂСѓР№С‚РµСЃСЊ РІ СЂРµР°Р»СЊРЅС‹С… СЃРёС‚СѓР°С†РёСЏС…</Text>
        </View>
        <Pressable onPress={() => dispatch(navigate('scenarios'))}><Text style={styles.all}>Р’СЃРµ вЂє</Text></Pressable>
      </View>

      <ScenarioCard scenario={scenarios[0]} onPress={() => dispatch(startScenario(scenarios[0].id))} />

      <Text style={styles.sectionTitle}>РџСЂРѕРіСЂРµСЃСЃ РЅРµРґРµР»Рё</Text>
      <View style={styles.weekCard}>
        <View style={styles.weekScore}>
          <Text style={styles.weekValue}>86%</Text>
          <Text style={styles.weekLabel}>С‚РѕС‡РЅРѕСЃС‚СЊ СЂРµС€РµРЅРёР№</Text>
        </View>
        <View style={styles.divider} />
        <View style={styles.statItem}><Text style={styles.statValue}>4</Text><Text style={styles.statLabel}>СЃС†РµРЅР°СЂРёСЏ</Text></View>
        <View style={styles.statItem}><Text style={styles.statValue}>38</Text><Text style={styles.statLabel}>РјРёРЅСѓС‚</Text></View>
      </View>
    </ScrollView>
  )
}
