export type AppScreen = 'auth' | 'home' | 'scenarios' | 'simulation' | 'debrief' | 'profile'

export interface Player { id: string; email: string; username: string; total_xp: number }
export interface Tokens { access_token: string; refresh_token: string; expires_in: number }
export interface AuthResult { player: Player; tokens: Tokens }
export interface Remark { code: string; xp: number; safety: number; loyalty: number; message: string }
