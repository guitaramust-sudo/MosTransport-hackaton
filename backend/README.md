...
Рейтинг симуляции использует отдельный реестр очков: 20 очков за первое
зачтённое прохождение семейства сценария и 10 за второй вариант. Повторы,
...
Сквозная проверка работающего демо API (создаёт две тестовые учётные записи):
...
Скрипт проверяет регистрацию, две ветки смены, повтор команды, изоляцию
аккаунтов, разбор, рейтинг, достижения, челлендж и уведомления.
...
| GET | `/api/session/simulations/{id}/result` | Причинный разбор завершённой демонстрационной смены |
...
Новый simulation API содержит один демонстрационный сценарий с двумя ветками,
двумя одновременными видимыми задачами и скрытым сигналом в багажной зоне.
`move_to` и `inspect` выполняются сервером и увеличивают игровое время.
Времена и эффекты — баланс прототипа, не норматив работы ВСМ. Демонстрационный
дедлайн услуги хранится на сервере; фоновая проверка переводит событие в ветку
пропущенного окна один раз. Итоговый debrief строится из журнала действий и
показывает влияние на шкалы. Старый диалоговый API остаётся доступным до
интеграции клиента. Достижения `first_complete` и `first_signal` выдаются
однократно после смены. Два зачтённых прохождения разных серверных вариантов
A/B в одной UTC-неделе дают один раз `+5 XP` и уведомление о завершении
челленджа. Уведомления о новом сценарии и старте челленджа создаются при
первом запуске демо игроком. Отдельные очки рейтинга — следующий этап.
